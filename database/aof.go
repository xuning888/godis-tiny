...
type Exec func(ctx context.Context, conn *redis.Client) error

type ForEach func(i int, fun func(key string, entity *DataEntity, expiration *time.Time) bool)
...
func (a *Aof) writeAof(dbIndex int, cmdLine CmdLine) {
	...
	if dbIndex != a.currentDb {
		...
		selectData := redis.MakeMultiBulkReply(selectCmd).ToBytes()
	...
	cmdLineData := redis.MakeMultiBulkReply(cmdLine).ToBytes()
	// 写入文件缓冲区
...
func (a *Aof) LoadAof(maxBytes int) {
	...
	ch := redis.DecodeInStream(reader)
	conn := redis.NewClient(0, nil, true)
	for p := range ch {
		if p.Error != nil {
			if p.Error == io.EOF {
				break
		...
		reply, ok := p.Data.(*redis.MultiBulkReply)
		if !ok {
