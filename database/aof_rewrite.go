...
func (a *Aof) DoRewrite(ctx *RewriteCtx) (err error) {
	...
	for i := 0; i < config.Properties.Databases; i++ {
		// select db
		data := redis.MakeMultiBulkReply(util.ToCmdLine("select", strconv.Itoa(i))).ToBytes()
		...
		// 将内存中的数据写入临时文件
		tmpAof.each(i, func(key string, entity *DataEntity, expiration *time.Time) bool {
...
func (a *Aof) FinishRewrite(ctx *RewriteCtx) {
	...
	errOccurs := func() bool {
		...
		selectDbBytes := redis.MakeMultiBulkReply(util.ToCmdLine("select", strconv.Itoa(ctx.dbIdx))).ToBytes()
	...
	selectBytes := redis.MakeMultiBulkReply(util.ToCmdLine("select", strconv.Itoa(a.currentDb))).ToBytes()
...
func EntityToCmd(key string, entity *DataEntity) *redis.MultiBulkReply {
	if entity == nil {
		return nil
	}
	var cmd *redis.MultiBulkReply
	switch entity.Type {
	case String:
...
func ExpireCmd(key string, expiration *time.Time) *redis.MultiBulkReply {
	cmdLine := util.MakeExpireCmd(key, *expiration)
	return redis.MakeMultiBulkReply(cmdLine)
}

var setCmd = []byte("set")

func stringToCmd(key string, bytes []byte) *redis.MultiBulkReply {
	args := make([][]byte, 3)
	args[0] = setCmd
	args[1] = []byte(key)
	args[2] = bytes
	return redis.MakeMultiBulkReply(args)
}

var pushCmd = []byte("rpush")

func listToCmd(key string, deque list.Dequeue) *redis.MultiBulkReply {
